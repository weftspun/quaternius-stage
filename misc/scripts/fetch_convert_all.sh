...
# Stream (pack \t folder_id) rows out of the parquet manifest via pixi's python.
rows() { pixi run python -c "import pandas as pd,sys
for r in pd.read_parquet(sys.argv[1]).itertuples(index=False):
    print(f'{r.pack}\t{r.folder_id}')" "$MANIFEST"; }

done=0; skipped=0; failed=""
while IFS=$'\t' read -r name fid; do
  [ -z "${fid:-}" ] || [ "$fid" = "nan" ] && { echo "NO-FOLDER $name"; failed="$failed $name(no-folder)"; continue; }
  out="models/$name"
